	...
	if ($('.section.started').length) {
		$(window).on('scroll', function () {
			...
			$.each(array, function (index) {
                ...
                if (!isLastElement)
                {
                    if (refElement.offset().top <= scrollPos + 100) //100 makes it trigger a bit faster
                    {
                ...
                {
                    //if (refElement.offset().top <= (scrollPos + window.innerHeight)) //this makes last section get activated the moment it's in view
                    if(scrollPos + window.innerHeight >= document.documentElement.scrollHeight - refElement.height())
                    {
					   $('.top-menu ul li').removeClass("active");
					   currLink.closest('li').addClass("active");
				    }
